use crate::{
    ...
    ui::{
        ...
        snippet::{
            dag_run_stats, dag_state_badge_type, head, header, system_stats, task_state_badge_type,
            task_stats,
        },
    },
};
...
pub async fn dag_runs(
    ...
    Ok(html! {
        ...
        html lang="en-US" {
            (head())
            body {
                (header("Dag Runs", &Some(system_id), &None, &None))
                main {
...
pub async fn tasks(
    ...
    Ok(html! {
        ...
        html lang="en-US" {
            (head())
            body {
                (header("Tasks", &Some(system_id), &Some(run_id), &None))
                main {
                    (system_stats(&system))
...
pub async fn logs(
    ...
    Ok(html! {
        ...
        html lang="en-US" {
            (head())
            body {
                (header("Tasks", &Some(system.system_id.clone()), &Some(run_id), &Some(task_id)))
                main {
                    (system_stats(&system))
